...
namespace PDFPrint
{
    public class WordHelper
    {
        ...
        private Word._Document _wordDocument;
        Word.WdExportFormat wdPdf = Microsoft.Office.Interop.Word.WdExportFormat.wdExportFormatPDF;
        ...
        public void Close()
        {
            ...
            if (_wordDocument != null )
            {
                ...
                _wordApplication.Quit(ref missing, ref missing, ref missing);
            }
        ...
        //    _wordApplication.Selection.Find.Text = oldStr;

        //    _wordApplication.Selection.Find.Replacement.ClearFormatting();
        //    _wordApplication.Selection.Find.Replacement.Text = newStr;

        //    _wordApplication.Selection.Find.Execute(
        ...
        //    #region 文本框
        //    StoryRanges sr = _wordDocument.StoryRanges;
        ...
        ///// 文本替换方法
        ///// </summary>
        ///// <param name="rePlaceStrList"></param>
        //public void SearchReplace(List<ReplaceStr> rePlaceStrList)
        //{
        //    object replaceAll = WdReplace.wdReplaceAll;
        ...
        //            do
        //            {
        //                foreach (var Str in rePlaceStrList)
        //                {
        //                    if (r1.Text.Contains(Str.oldStr))
        //                    {
        //                        if (Str.strType == 0)
        //                        {
        //                            r1.Find.ClearFormatting();
        //                            r1.Find.Text = Str.oldStr;
        //                            r1.Find.Replacement.ClearFormatting();
        //                            r1.Find.Replacement.Text = Str.newStr;
        //                            r1.Find.Execute(
        //                            ref missing, ref missing, ref missing, ref missing, ref missing,
        //                            ref missing, ref missing, ref missing, ref missing, ref missing,
        //                            ref replaceAll, ref missing, ref missing, ref missing, ref missing);
        ...
        //                            r1.Text = "";
        //                            r1.Select();
        //                            InsertPic(Str.newStr, Str.Length, Str.Hight);
        //                            File.Delete(Str.newStr);
        //                        }
        //                    }
        //                }
        //                r1 = r1.NextStoryRange;
        ...
        /// 解析文件
        /// </summary>
        public bool GetCharByUse(ref List<string> Mark)
        {
            string marktext = WordDocument.Content.Text.ToString();
            string oldmark;
            try
            {
                //处理标记文本
                while (marktext.IndexOf("[@") != -1)
                {
                    //如果起始标记在终止标记之后则跳出循环（起始标记出错）
                    if (marktext.IndexOf("@]") < marktext.IndexOf("[@"))
                    {
                        return false;
                    }

                    oldmark = marktext.Substring(marktext.IndexOf("[@"), marktext.IndexOf("@]") - marktext.IndexOf("[@") + 2);

                    //如果截取标记段落中含有起始标记则跳出循环（终止标记出错）
                    if (oldmark.Substring(2).IndexOf("[@") != -1)
                    {
                        return false;
                    }

                    marktext = marktext.Substring(marktext.IndexOf("@]") + 2);
                    //插入文字区域标记
                    Mark.Add(oldmark);
                }
                StoryRanges sr = _wordDocument.StoryRanges;
                foreach (Range r in sr)
                {
                    ...
                    if (WdStoryType.wdTextFrameStory == r.StoryType)
                    {
                        ...
                        {
                            //插入文本框标记
                            if (r1.Text.IndexOf("[@") != -1)
                            {
                                Mark.Add(r1.Text.Substring(r1.Text.IndexOf("[@"), r1.Text.IndexOf("@]") - r1.Text.IndexOf("[@") + 2));
                            }
                            else if (r1.Text.IndexOf("[QR") != -1)
                            {
                                Mark.Add(r1.Text.Substring(r1.Text.IndexOf("[QR"), r1.Text.IndexOf("QR]") - r1.Text.IndexOf("[QR") + 3));
                            }
                            else if (r1.Text.IndexOf("[B") != -1)
                            {
                                Mark.Add(r1.Text.Substring(r1.Text.IndexOf("[B"), r1.Text.IndexOf("B]") - r1.Text.IndexOf("[B") + 2));
                            }
                            r1 = r1.NextStoryRange;
                        } while (r1 != null);
                    }
        ...
        /// 查找并替换二维码
        /// </summary>
        /// <param name="fileName">二维码路径</param>
        /// <param name="replace">替换的标记字段</param>
        /// <param name="With">图片的宽度</param>
        /// <param name="Hight">图片的高度</param>
        public void replaceQRPic(string fileName, string replace, float With, float Hight)
        {
            StoryRanges sr = _wordDocument.StoryRanges;
            foreach (Range r in sr)
            {
                ...
                if (WdStoryType.wdTextFrameStory == r.StoryType)
                {
                    do
                    {
                        if (r1.Text.IndexOf(replace) > -1)
                        {
                            r1.Text = "";
                            r1.Select();
                            InsertPic(fileName, With, Hight);
                        }
                        r1 = r1.NextStoryRange;
                    } while (r1 != null);
                }
